...
async fn process(goodreads_book_url: &str) -> Result<DownloadLinks, Error> {
    ...
    Ok(library::get_download_links(book_metadata.md5.as_str()).await?)
}
...
async fn test_process() {
    ...
    let got = process(test_url).await.expect("Should get download links");
